...
<style>
    ...
    .tab-btn { padding:12px 24px; border:none; background:none; font-size:0.95rem; font-weight:600;
               color:#64748B; cursor:pointer; border-bottom:3px solid transparent; margin-bottom:-2px;
               transition:all 0.2s; display:inline-flex; align-items:center; gap:8px; }
    .tab-btn.active { color:#246BCE; border-bottom-color:#246BCE; }
    .tab-btn:hover:not(.active) { color:#246BCE; }
    .tab-content { display:none; } .tab-content.active { display:block; animation:fadeIn 0.3s ease; }
    @keyframes fadeIn { from{opacity:0;transform:translateY(8px)} to{opacity:1;transform:translateY(0)} }

    /* Form section */
    .form-section { background:#fff; border-radius:12px; padding:28px; margin-bottom:28px;
                    box-shadow:0 2px 8px rgba(0,0,0,0.06); }
    .section-title { font-size:1.25rem; font-weight:700; color:#1E293B; margin-bottom:22px;
                     display:flex; align-items:center; gap:10px; }
    .section-title i { color:#246BCE; }
    ...
    .form-group input, .form-group select, .form-group textarea {
        ...
        font-size:0.95rem; color:#1F2937; transition:all 0.2s; background:#fff; width:100%; }
    ...
    /* Buttons */
    .btn { padding:10px 22px; border:none; border-radius:8px; font-size:0.9rem; font-weight:600;
    ...
    .jadwal-card { background:#fff; border-radius:12px; padding:20px; border-left:4px solid #10B981;
                   box-shadow:0 2px 8px rgba(0,0,0,0.07); transition:all 0.3s ease; }
    .jadwal-card:hover { transform:translateY(-2px); box-shadow:0 6px 16px rgba(0,0,0,0.12); }
    .jadwal-card.imunisasi { border-left-color:#246BCE; }
    .jadwal-card.selesai   { border-left-color:#6B7280; }
    .jadwal-card.dibatalkan{ border-left-color:#EF4444; }

    .jadwal-status-badge { display:inline-flex; align-items:center; gap:5px; padding:4px 12px;
                           border-radius:20px; font-size:0.72rem; font-weight:700; text-transform:uppercase;
                           margin-bottom:10px; }
    .badge-terjadwal  { background:#10B981; color:#fff; }
    .badge-selesai    { background:#6B7280; color:#fff; }
    ...
    .info-row i { color:#246BCE; width:16px; flex-shrink:0; margin-top:2px; }
...
<div class="main-card">
    ...
    <div>
        <h1 class="page-title">Jadwal Bulanan Posyandu/Imunisasi</h1>
        <p class="page-subtitle">Kelola jadwal dan tracking imunisasi anak</p>
    </div>
    ...
    <div id="tab-jadwal" class="tab-content active">
            ...
            <form id="formJadwal" onsubmit="submitJadwal(event)">
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <input type="text" id="namaKegiatan" placeholder="Contoh: Posyandu Rutin Bulan November" required>
                    </div>
                    <div class="form-group">
                        <label>Tanggal</label>
                        <input type="date" id="tanggalJadwal" required>
                        <small class="error-hint" id="errorTanggal">❌ Tanggal tidak boleh di masa lalu!</small>
                    </div>
                </div>
                <div class="form-row">
                    <div class="form-group">
                ...
                <div class="form-group">
                    <label>Keterangan &amp; Catatan Khusus</label>
                    <textarea id="keteranganJadwal" placeholder="Catatan tambahan untuk jadwal ini..."></textarea>
                </div>
                <div class="form-actions">
                    <button type="submit" class="btn btn-success" id="btnSubmitJadwal">
                        <i class="fas fa-check-circle"></i> Buat Jadwal
                    </button>
                    <button type="button" class="btn btn-secondary" onclick="resetFormJadwal()">
        ...
        <div class="form-section">
            ...
            <div class="jadwal-grid" id="jadwalCardsContainer">
    ...
    <div id="tab-imunisasi" class="tab-content">
        ...
        <div class="form-section">
            ...
            <form id="formImunisasi" onsubmit="submitImunisasi(event)">
                ...
                <div class="form-row">
                    <div class="form-group">
                        <label>Tanggal</label>
                        <input type="date" id="tanggalImunisasi" required>
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <select id="jenisImunisasi" required>
                ...
                <div class="form-group">
                    <label>Keterangan &amp; Catatan Khusus</label>
                    <textarea id="keteranganImunisasi" placeholder="Catatan tambahan untuk jadwal ini..."></textarea>
                </div>
                <div class="form-actions">
                    <button type="submit" class="btn btn-success" id="btnSubmitImunisasi">
                        <i class="fas fa-check-circle"></i> Buat Jadwal
                    </button>
                    <button type="button" class="btn btn-secondary" onclick="resetFormImunisasi()">
                        <i class="fas fa-times"></i> Cancel / Reset
                    </button>
                </div>
            </form>
        ...
        <div class="form-section">
            ...
            <div class="jadwal-grid" id="imunisasiCardsContainer">
                <div class="empty-state">
                    <i class="fas fa-spinner fa-spin"></i>
                    <p>Memuat data...</p>
                </div>
...
<div class="modal-overlay" id="modalHapus">
    <div class="modal-box">
        ...
        <p style="color:#374151;margin-bottom:20px;">Hapus jadwal <strong id="hapusNama"></strong>? Tindakan ini tidak dapat dibatalkan.</p>
...
function buildCard(j, isImunisasi = false) {
    ...
    const cardClass  = isImunisasi ? 'imunisasi' : (status === 'Selesai' ? 'selesai' : status === 'Dibatalkan' ? 'dibatalkan' : '');
    const icon       = isImunisasi ? '💉' : '✅';

    const keteranganHtml = j.keterangan ? `
        <div class="info-row keterangan-box">
            <i class="fas fa-sticky-note"></i>
            <span><span class="keterangan-label">📝 Catatan:</span>${j.keterangan}</span>
        </div>` : '';

    return `
        <div class="jadwal-card ${cardClass}" id="card-${j.id}">
            ...
            <div class="jadwal-actions">
                <button class="btn btn-primary btn-sm" onclick='openEdit(${JSON.stringify(j)}, ${isImunisasi})'>
                    <i class="fas fa-edit"></i> Edit
                </button>
                <button class="btn btn-danger btn-sm" onclick="confirmHapus(${j.id}, '${j.nama_kegiatan.replace(/'/g,"\\'")}')">
...
async function loadJadwal() {
    try {
        const res  = await fetch('<?php echo e(route("jadwal.list")); ?>?layanan=balita', {
...
function renderCards() {
    const jadwalEl    = document.getElementById('jadwalCardsContainer');
    const imunisasiEl = document.getElementById('imunisasiCardsContainer');

    jadwalEl.innerHTML = allJadwal.length
        ? allJadwal.map(j => buildCard(j, false)).join('')
        : `<div class="empty-state"><i class="fas fa-calendar-times"></i><h3>Belum Ada Jadwal</h3><p>Buat jadwal baru menggunakan form di atas</p></div>`;

    imunisasiEl.innerHTML = allImunisasi.length
        ? allImunisasi.map(j => buildCard(j, true)).join('')
        : `<div class="empty-state"><i class="fas fa-syringe"></i><h3>Belum Ada Jadwal Imunisasi</h3><p>Buat jadwal imunisasi baru menggunakan form di atas</p></div>`;
}

// ── Load vaksin untuk dropdown ─────────────────────────────
async function loadVaksin() {
    try {
        const res  = await fetch('<?php echo e(route("vaksin.list")); ?>', {
            headers: { 'X-CSRF-TOKEN': CSRF_TOKEN, 'Accept': 'application/json' },
            credentials: 'same-origin'
        });
        const data = await res.json();
        const sel  = document.getElementById('jenisImunisasi');
        (data.data || []).forEach(v => {
            const opt = document.createElement('option');
            opt.value = v.nama_vaksin;
            opt.textContent = v.nama_vaksin;
            sel.appendChild(opt);
        });
    } catch (e) {}
}

// ── Submit Jadwal Posyandu ─────────────────────────────────
async function submitJadwal(e) {
    ...
    if (!isEdit && tgl && new Date(tgl) < new Date(new Date().toDateString())) {
        errTgl.style.display = 'block'; return;
    }
    ...
    try {
        ...
        const data = await res.json();
        if (data.success) {
            toast(data.message || (isEdit ? 'Jadwal diperbarui!' : 'Jadwal berhasil dibuat!'), 'success');
            resetFormJadwal();
            loadJadwal();
        } else {
        ...
        btn.disabled = false;
        btn.innerHTML = '<i class="fas fa-check-circle"></i> ' + (isEdit ? 'Simpan Perubahan' : 'Buat Jadwal');
    }
}

// ── Submit Jadwal Imunisasi ────────────────────────────────
async function submitImunisasi(e) {
    ...
    try {
        ...
        const data = await res.json();
        if (data.success) {
            toast(data.message || (isEdit ? 'Jadwal diperbarui!' : 'Jadwal imunisasi berhasil dibuat!'), 'success');
            resetFormImunisasi();
            loadJadwal();
        } else {
            toast(data.message || 'Gagal menyimpan', 'error');
...
function openEdit(j, isImunisasi) {
    if (isImunisasi) {
        ...
        // Set jenis imunisasi dari nama_kegiatan
        const jenisMatch = j.nama_kegiatan.replace('Imunisasi ', '');
        const sel = document.getElementById('jenisImunisasi');
        for (let opt of sel.options) { if (opt.value === jenisMatch) { sel.value = jenisMatch; break; } }
        document.getElementById('btnSubmitImunisasi').innerHTML = '<i class="fas fa-check-circle"></i> Simpan Perubahan';
        document.querySelector('#formImunisasi .section-title, #tab-imunisasi .section-title');
        window.scrollTo({ top: document.getElementById('tab-imunisasi').offsetTop - 80, behavior: 'smooth' });
    } else {
        switchTab('jadwal');
...
function resetFormJadwal() {
    ...
    document.getElementById('btnSubmitJadwal').innerHTML = '<i class="fas fa-check-circle"></i> Buat Jadwal';
    document.getElementById('errorTanggal').style.display = 'none';
}
function resetFormImunisasi() {
    document.getElementById('formImunisasi').reset();
    document.getElementById('imunisasiId').value = '';
    document.getElementById('btnSubmitImunisasi').innerHTML = '<i class="fas fa-check-circle"></i> Buat Jadwal';
    document.getElementById('errorTanggalImunisasi').style.display = 'none';
}
...
<?php echo $__env->make('layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\Users\asus\VSCode\poscare-laravel\resources\views/jadwal/index.blade.php ENDPATH**/ ?>