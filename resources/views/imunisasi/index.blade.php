...
<div class="modal-overlay" id="modalTandai">
    <div class="modal-box">
        ...
        <div class="form-group">
            ...
            <input type="date" id="tanggalImunisasi" max="{{ date('Y-m-d') }}" value="{{ date('Y-m-d') }}"
...
{{-- ── Modal: Konfirmasi Hapus Kolom Vaksin ── --}}
...
async function loadImunisasi() {
    ...
    try {
        const url = `{{ route('imunisasi.list') }}${search ? '?q=' + encodeURIComponent(search) : ''}`;
...
document.getElementById('btnKonfirmasiTandai').addEventListener('click', async () => {
    ...
    try {
        const res  = await fetch('{{ route("imunisasi.tandai") }}', {
            method: 'POST',
...
document.getElementById('btnKonfirmasiHapusImun').addEventListener('click', async () => {
    ...
    try {
        const res  = await fetch('{{ route("imunisasi.undo") }}', {
            method: 'POST',
            headers: { 'Content-Type':'application/json', 'X-CSRF-TOKEN':CSRF_TOKEN, 'Accept':'application/json' },
            credentials: 'same-origin',
...
async function simpanVaksinBaru() {
    ...
    btn.disabled = true; btn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Menyimpan...';
    try {
        const res  = await fetch('{{ route("vaksin.store") }}', {
            method: 'POST',
            headers: { 'Content-Type':'application/json', 'X-CSRF-TOKEN':CSRF_TOKEN, 'Accept':'application/json' },
            credentials: 'same-origin',
