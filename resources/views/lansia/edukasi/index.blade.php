...
<style>
    ...
    .tab-nav-top { display:flex; gap:0; margin-bottom:28px; background:#fff; border-radius:12px;
                   box-shadow:0 2px 8px rgba(6,78,59,0.08); overflow:hidden; }
    ...
    .tab-nav-top .tab-btn:hover:not(.active) { background:#F0FDF4; color:#10B981; }
    .tab-content { display:none; } .tab-content.active { display:block; }

    /* ── Card Grid ── */
    .card-grid { display:grid; grid-template-columns:repeat(auto-fill,minmax(280px,1fr)); gap:20px; }
    .edu-card { background:#fff; border-radius:14px; box-shadow:0 4px 16px rgba(6,78,59,0.08);
                overflow:hidden; transition:transform 0.2s,box-shadow 0.2s; display:flex; flex-direction:column;
                border:1px solid rgba(16,185,129,0.1); }
    .edu-card:hover { transform:translateY(-4px); box-shadow:0 8px 28px rgba(6,78,59,0.14); }
    .edu-thumb-wrap { position:relative; width:100%; height:170px; overflow:hidden; background:#D1FAE5; }
    .edu-thumb-wrap img { width:100%; height:100%; object-fit:cover; display:block; }
    .edu-thumb-placeholder { width:100%; height:100%; display:flex; align-items:center; justify-content:center;
                              font-size:52px; background:linear-gradient(135deg,#D1FAE5,#A7F3D0); }
    ...
    .edu-cat-label { font-size:11px; font-weight:700; color:#065F46; text-transform:uppercase; letter-spacing:0.5px; }
    .edu-card-title { font-size:14px; font-weight:700; color:#064E3B; line-height:1.4;
                      display:-webkit-box; -webkit-line-clamp:2; -webkit-box-orient:vertical; overflow:hidden; }
    .edu-card-date { font-size:12px; color:#6B7280; display:flex; align-items:center; gap:5px; margin-top:auto; }
    ...
    .filter-group label { font-size:12px; font-weight:600; color:#065F46; }
    .filter-group select { padding:8px 12px; border:1px solid #A7F3D0; border-radius:8px; font-size:13px;
                           background:#fff; transition:all 0.2s; }
    .filter-group select:focus { outline:none; border-color:#10B981; box-shadow:0 0 0 3px rgba(16,185,129,0.1); }

    /* ── Kelola Konten ── */
    .kelola-card { background:#fff; border-radius:16px; padding:28px; box-shadow:0 4px 16px rgba(6,78,59,0.08);
                   margin-bottom:24px; border:1px solid rgba(16,185,129,0.1); }
    .kelola-title { font-size:1.1rem; font-weight:700; color:#064E3B; margin-bottom:20px;
    ...
    .form-group input, .form-group select, .form-group textarea {
        padding:10px 14px; border:1px solid #A7F3D0; border-radius:8px; font-size:14px;
    ...
    .platform-hint { font-size:12px; color:#6B7280; margin-top:4px; display:flex; align-items:center; gap:5px; }
    ...
    .konten-thumb-sm-placeholder { width:60px; height:44px; border-radius:6px; background:#D1FAE5;
                                   display:flex; align-items:center; justify-content:center;
                                   font-size:20px; flex-shrink:0; }
    .konten-info { flex:1; min-width:0; }
    .konten-title { font-size:14px; font-weight:600; color:#064E3B;
                    white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
    .konten-meta { font-size:12px; color:#6B7280; margin-top:2px; }
    ...
    <div class="filter-row">
        ...
        <div class="filter-group">
            ...
            <select id="filterCategory" onchange="loadEdukasi()">
                ...
                <option value="tips-lansia">Tips Kesehatan Lansia</option>
            </select>
        </div>
    </div>
    <div id="edukasiGrid" class="card-grid">
        <div class="loading-center" style="grid-column:1/-1;">
            <i class="fas fa-spinner fa-spin fa-2x" style="color:#10B981;"></i><br><br>Memuat konten...
        </div>
    </div>
</div>
...
<div id="tab-kelola" class="tab-content">
    <div class="kelola-card">
        ...
        <form id="formEdukasi" onsubmit="submitEdukasi(event)">
            ...
            <div class="form-group">
                ...
                <small class="platform-hint info"><i class="fas fa-info-circle"></i> Auto-fetch tersedia untuk YouTube &amp; Artikel</small>
            </div>
            <div class="form-group">
    ...
    <div class="kelola-card">
        ...
        <div id="kontenList">
            <div class="loading-center"><i class="fas fa-spinner fa-spin" style="color:#10B981;"></i> Memuat...</div>
        </div>
    </div>
</div>
@endif
...
@section('scripts')
<script>
const LAYANAN = 'lansia'; // semua request pakai layanan=lansia
...
const PLATFORM_HINTS = {
    youtube:   {cls:'info', text:'<i class="fas fa-info-circle"></i> Format: youtube.com/watch?v=... | youtu.be/...'},
    tiktok:    {cls:'info', text:'<i class="fas fa-info-circle"></i> Pastikan video publik'},
    facebook:  {cls:'warn', text:'<i class="fas fa-exclamation-triangle"></i> HANYA konten Facebook publik'},
    instagram: {cls:'warn', text:'<i class="fas fa-exclamation-triangle"></i> HANYA konten Instagram publik'},
    article:   {cls:'info', text:'<i class="fas fa-info-circle"></i> URL artikel dari website manapun'},
};

const CAT_LABEL = {
    'kesehatan-lansia':'KESEHATAN LANSIA','pola-hidup-sehat':'POLA HIDUP SEHAT',
    'pencegahan-penyakit':'PENCEGAHAN PENYAKIT','gizi-lansia':'GIZI LANSIA','tips-lansia':'TIPS KESEHATAN LANSIA',
};
...
function onPlatformChange() {
    ...
    if(PLATFORM_CONFIG[p]) urlInput.placeholder=PLATFORM_CONFIG[p].placeholder;
    if(PLATFORM_HINTS[p]){
        hint.className=`platform-hint ${PLATFORM_HINTS[p].cls}`;
        hint.innerHTML=PLATFORM_HINTS[p].text;
...
function buildEduCard(item) {
    ...
    const catLabel=CAT_LABEL[item.category]||(item.category||'').toUpperCase();
    ...
    return `<div class="edu-card">
        <div class="edu-thumb-wrap">
            ${thumbHtml}
            <span class="platform-badge ${cfg.badgeClass}"><i class="${cfg.icon}"></i> ${cfg.label}</span>
        </div>
...
async function loadEdukasi() {
    ...
    grid.innerHTML=`<div class="loading-center" style="grid-column:1/-1;"><i class="fas fa-spinner fa-spin fa-2x" style="color:#10B981;"></i><br><br>Memuat konten...</div>`;

    const params=new URLSearchParams({layanan:LAYANAN});
    if(platform) params.append('platform',platform);
    if(category) params.append('category',category);

    try {
        const res=await fetch(`{{ route('edukasi.list') }}?${params.toString()}`,{
...
async function loadKontenList() {
    ...
    el.innerHTML='<div class="loading-center"><i class="fas fa-spinner fa-spin" style="color:#10B981;"></i> Memuat...</div>';
    try {
        const res=await fetch(`{{ route('edukasi.list') }}?layanan=${LAYANAN}`,{
            headers:{'X-CSRF-TOKEN':CSRF_TOKEN,'Accept':'application/json'},credentials:'same-origin'
        });
        const data=await res.json();
        const list=data.data||[];
        if(!list.length){el.innerHTML='<div style="text-align:center;padding:30px;color:#9CA3AF;">Belum ada konten</div>';return;}
        el.innerHTML=list.map(item=>{
            ...
            return `<div class="konten-list-item">
                ${thumbHtml}
                <div class="konten-info">
                    <div class="konten-title">${item.title}</div>
                    <div class="konten-meta">• ${CAT_LABEL[item.category]||item.category||'-'}</div>
                    <a href="${item.url}" target="_blank" class="konten-url">${item.url}</a>
                </div>
                <div class="konten-actions">
                    <button class="btn btn-primary btn-sm" onclick="editKonten(${item.id})"><i class="fas fa-edit"></i></button>
                    <button class="btn btn-danger btn-sm" onclick="hapusKonten(${item.id},'${item.title.replace(/'/g,"\\'")}')"><i class="fas fa-trash"></i></button>
                </div>
            </div>`;
        }).join('');
    } catch(e) {
...
async function fetchInfo() {
    ...
    if(!['youtube','article'].includes(platform)){toast('Auto-fetch hanya untuk YouTube dan Artikel','warning');return;}
    btn.disabled=true; btn.innerHTML='<i class="fas fa-spinner fa-spin"></i>';
    try {
        const res=await fetch('{{ route("edukasi.fetchInfo") }}',{
            method:'POST',headers:{'Content-Type':'application/json','X-CSRF-TOKEN':CSRF_TOKEN,'Accept':'application/json'},
            credentials:'same-origin',body:JSON.stringify({platform,url}),
        });
        const data=await res.json();
        ...
        } else toast('Tidak dapat auto-fetch. Isi judul secara manual.','warning');
    ...
    try {
        ...
        btn.onclick=async(e)=>{
            ...
            try {
                ...
                if(d.success!==false&&r.ok){toast('Konten berhasil diperbarui!','success');resetForm();loadKontenList();loadEdukasi();}
...
async function hapusKonten(id, nama) {
    ...
    try {
        const res=await fetch(`{{ url('/api/edukasi') }}/${id}`,{
            method:'DELETE',headers:{'X-CSRF-TOKEN':CSRF_TOKEN,'Accept':'application/json'},credentials:'same-origin'
        });
        const data=await res.json();
        if(data.success){toast('Konten berhasil dihapus!','success');loadKontenList();loadEdukasi();}
