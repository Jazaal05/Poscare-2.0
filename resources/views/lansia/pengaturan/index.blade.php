...
<style>
    ...
    .card { background:#fff; border-radius:16px; padding:24px; box-shadow:0 6px 24px rgba(6,78,59,0.08); margin-bottom:24px; }
    .page-title { font-size:26px; font-weight:700; color:#064E3B; margin-bottom:4px; }
    .page-subtitle { font-size:14px; color:#6B7280; margin-bottom:24px; }

    .posyandu-header { background:linear-gradient(135deg,#065F46,#10B981); border-radius:16px; padding:28px; margin-bottom:24px; color:#fff; text-align:center; }
    ...
    .card-title { font-size:18px; font-weight:700; color:#064E3B; margin-bottom:20px; padding-bottom:12px; border-bottom:2px solid #D1FAE5; display:flex; align-items:center; gap:10px; }

    .form-group { display:flex; flex-direction:column; gap:6px; margin-bottom:16px; }
    .form-group label { font-size:13px; font-weight:600; color:#374151; }
    .form-group input { padding:10px 14px; border:2px solid #D1FAE5; border-radius:8px; font-size:14px; transition:all 0.2s; width:100%; box-sizing:border-box; }
    .form-group input:focus { outline:none; border-color:#10B981; box-shadow:0 0 0 3px rgba(16,185,129,0.1); }
    .form-group input[readonly] { background:#F0FDF4; color:#94A3B8; cursor:not-allowed; }
    ...
    thead th { background:#F0FDF4; padding:12px 16px; text-align:left; font-weight:600; color:#065F46; border-bottom:2px solid #D1FAE5; white-space:nowrap; }
    tbody td { padding:12px 16px; border-bottom:1px solid #ECFDF5; color:#374151; vertical-align:middle; }
    ...
    .modal-header { display:flex; justify-content:space-between; align-items:center; margin-bottom:20px; padding-bottom:16px; border-bottom:2px solid #D1FAE5; }
    .modal-title { font-size:18px; font-weight:700; color:#064E3B; }
...
<div style="margin-bottom:20px;">
    <h1 class="page-title"><i class="fas fa-cog" style="color:#10B981;"></i> Pengaturan</h1>
...
<div class="cards-row">
    ...
    <div class="card">
        ...
        <form id="formProfil" onsubmit="submitProfil(event)">
            <div class="form-group">
                ...
                <input type="text" id="nama_lengkap" placeholder="Nama lengkap Anda">
            </div>
            <div class="form-group">
                <label>Email</label>
            ...
            <div class="form-group">
                ...
                <input type="text" id="no_telp" placeholder="08xxxxxxxxxx">
            </div>
    ...
    <div class="card">
        ...
        <div class="info-box">
            ...
            <span>Kode OTP akan dikirim ke email Anda untuk verifikasi perubahan kata sandi.</span>
        </div>
        <div id="step1OTP">
            <div class="form-group">
...
<div class="modal-overlay" id="modalHapusUser">
    <div class="modal-box">
        ...
        <p style="color:#374151;margin-bottom:20px;">Hapus pengguna <strong id="hapusUserNama"></strong>?</p>
...
async function loadCurrentUser() {
    try {
        const res  = await fetch('{{ route("pengaturan.currentUser") }}', {
...
async function submitProfil(e) {
    ...
    try {
        const res  = await fetch('{{ route("pengaturan.profil") }}', {
...
async function requestOTP() {
    ...
    try {
        const res  = await fetch('{{ route("pengaturan.requestOtp") }}', {
        ...
        } else toast(data.message||'Gagal mengirim OTP','error');
    } catch(e) { toast('Koneksi gagal','error'); }
...
async function verifikasiOTP() {
    ...
    try {
        const res  = await fetch('{{ route("pengaturan.verifikasiOtp") }}', {
            method:'POST', credentials:'same-origin',
            headers:{'Content-Type':'application/json','X-CSRF-TOKEN':CSRF_TOKEN,'Accept':'application/json'},
            body: JSON.stringify({otp, password_baru:pass, password_baru_confirmation:confirm}),
        });
        const data = await res.json();
        if (data.success) {
            toast('Password berhasil diubah!','success');
            backToStep1();
            ['otpCode','passwordBaru','passwordBaruConfirm'].forEach(id=>document.getElementById(id).value='');
        } else toast(data.message||'Gagal verifikasi OTP','error');
    } catch(e) { toast('Koneksi gagal','error'); }
...
async function loadUsers() {
    ...
    try {
        const res  = await fetch('{{ route("pengaturan.users") }}', {
            headers:{'X-CSRF-TOKEN':CSRF_TOKEN,'Accept':'application/json'}, credentials:'same-origin'
        });
        const data = await res.json();
        const list = data.data||[];
        if (!list.length) { tbody.innerHTML='<tr><td colspan="5" style="text-align:center;padding:30px;color:#9CA3AF;">Tidak ada pengguna</td></tr>'; return; }
        const currentId = {{ Auth::id() }};
        tbody.innerHTML = list.map((u,i)=>`
            <tr>
                <td>${i+1}</td>
                <td><strong>${u.username||'-'}</strong>${u.id===currentId?'<span class="badge badge-info" style="margin-left:6px;font-size:10px;">Anda</span>':''}</td>
                <td>${u.email||'-'}</td>
                <td>${roleBadge(u.role)}</td>
                <td>${u.id!==currentId?`<button class="btn btn-danger btn-sm" onclick="confirmHapusUser(${u.id},'${(u.username||'').replace(/'/g,"\\'")}')"><i class="fas fa-trash"></i></button>`:'<span style="color:#9CA3AF;font-size:12px;">—</span>'}</td>
            </tr>`).join('');
