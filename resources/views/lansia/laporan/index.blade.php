...
.tips-box { display:flex; align-items:flex-start; gap:10px; background:#ECFDF5; border-left:4px solid #10B981; border-radius:8px; padding:12px 16px; margin-bottom:24px; font-size:13px; color:#065F46; }
...
.filter-label { font-size:11px; font-weight:700; color:#1E3A5F; text-transform:uppercase; letter-spacing:0.5px; display:flex; align-items:center; gap:6px; }
.filter-label i { color:#10B981; }
.filter-input { width:100%; padding:11px 14px; background:#F8FAFC; border:1px solid #E2E8F0; border-radius:10px; font-size:14px; color:#1E3A5F; transition:all 0.2s; box-sizing:border-box; }
.filter-input:focus { outline:none; border-color:#10B981; background:#fff; box-shadow:0 0 0 3px rgba(16,185,129,0.1); }
.format-section-title { font-size:14px; font-weight:600; color:#374151; margin-bottom:14px; display:flex; align-items:center; gap:8px; }
.format-section-title i { color:#10B981; }
.format-card { border:2px solid #10B981; border-radius:12px; padding:20px 24px; display:flex; flex-direction:column; align-items:center; gap:8px; background:linear-gradient(135deg,#F0FDF4,#ECFDF5); max-width:220px; cursor:pointer; transition:all 0.2s; }
...
.btn-download { display:flex; align-items:center; justify-content:center; gap:10px; padding:14px 40px; background:#10B981; color:#fff; border:none; border-radius:10px; font-size:15px; font-weight:700; cursor:pointer; transition:all 0.2s; margin-top:24px; width:100%; max-width:320px; margin-left:auto; margin-right:auto; }
.btn-download:hover { background:#059669; transform:translateY(-2px); box-shadow:0 6px 16px rgba(16,185,129,0.35); }
.btn-download:disabled { opacity:0.6; cursor:not-allowed; transform:none; }
.riwayat-card { background:#fff; border-radius:16px; padding:24px 28px; box-shadow:0 4px 20px rgba(0,0,0,0.08); }
.riwayat-title { font-size:16px; font-weight:700; color:#1E3A5F; margin-bottom:16px; display:flex; align-items:center; gap:8px; }
.riwayat-title i { color:#10B981; }
.riwayat-item { display:flex; align-items:center; gap:14px; padding:14px 0; border-bottom:1px solid #F1F5F9; }
...
.meta-badge { display:inline-flex; align-items:center; gap:4px; padding:2px 8px; border-radius:12px; font-size:11px; font-weight:600; }
...
<div>
    <h1 class="page-title">Laporan &amp; Export Data Lansia</h1>
    <p class="page-subtitle">Ekspor data lansia dalam format Excel</p>
</div>

<div class="main-card">
    <div class="card-section-title">Filter Data Export</div>
    <div class="card-section-sub">Pilih rentang waktu dan kategori data yang ingin diekspor</div>

    <div class="tips-box">
        <i class="fas fa-lightbulb"></i>
        <div>
            <strong>Tips Cerdas</strong><br>
            Pilih rentang tanggal dan kategori yang ingin di-export. Tersedia tiga kategori: Data Lansia (biodata), Data Pemeriksaan (riwayat kesehatan), dan Data Pengobatan (obat &amp; vitamin).
        </div>
    </div>
    ...
    <div class="format-section-title">
        <i class="fas fa-download"></i> Format Export
    </div>
    <div class="format-card">
        <div class="format-icon"><i class="fas fa-file-excel"></i></div>
        <div class="format-name">Excel (.xlsx)</div>
        <div class="format-desc">Kompatibel dengan Excel &amp; Google Sheets</div>
    </div>
...
@section('scripts')
<script>
const EXPORT_URL  = '{{ route("lansia.laporan.export") }}';
const RIWAYAT_KEY = 'poscare_lansia_export_history';
...
function renderRiwayat() {
    ...
    const catLabel = { lansia:'Data Lansia', pemeriksaan:'Data Pemeriksaan', pengobatan:'Data Pengobatan' };
...
async function exportExcel() {
    ...
    try {
        ...
        const catMap = { lansia:'Data_Lansia', pemeriksaan:'Data_Pemeriksaan', pengobatan:'Data_Pengobatan' };
        a.href     = url;
        a.download = `PosCare_Lansia_${catMap[category]||'Laporan'}_${start_date}_${end_date}.xlsx`;
